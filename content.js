...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'clickAskMoreButton') {
    ...
    if (!pathname.startsWith('/subject/') && !pathname.startsWith('/search/')) {
      sendResponse({ success: false, error: '当前页面不支持追问操作' });
      return true;
    }
    ...
    if (askMoreButton) {
      ...
      sendResponse({ success: false, error: '未找到追问按钮' });
    }
    return true;
  }
  ...
  if (message.action === 'clickHomeButton') {
    ...
    if (pathname === '/' || pathname === '') {
      sendResponse({ success: false, error: '在首页不需要触发返回主页操作' });
      return true;
    }
    ...
    const homeButton = document.querySelector('button.MuiButtonBase-root[tabindex="0"][aria-label="主页"]');
    if (homeButton) {
      // 创建并分发自定义点击事件
      const clickEvent = new MouseEvent('click', {
        bubbles: true,
      ...
      sendResponse({ success: false, error: '未找到主页按钮' });
    }
  }
  return true;
});